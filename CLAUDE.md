...
- Photo (`/assets/cta-treadmill.png` — two people running on treadmills; swapped in from an updated Figma frame, replacing the earlier meditation photo): absolutely positioned box `w-[57.03%]` flush right (= 730/1280 from the Figma frame), `object-cover`, and `unoptimized` on the `next/image` — the source is a small export (under 700px wide), so Next's automatic upscaling for retina `sizes` produces a soft/blurry result; `unoptimized` serves the raw file and lets the browser do a single clean scale instead. If the source photo changes again and needs bottom-anchored cropping (not this one), add `object-bottom` back.
